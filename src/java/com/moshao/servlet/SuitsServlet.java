...
public class SuitsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            List<Product> suits = productDao.getSuits();
            // Set suits attribute in request for JSP rendering
            request.setAttribute("suits", suits);
            // Forward request to suits.jsp for rendering
            request.getRequestDispatcher("suits.jsp").forward(request, response);
        } catch (SQLException e) {
            // Log SQL exceptions
            logger.log(Level.SEVERE, "SQL Exception", e);
            // Handle database error
            response.getWriter().println("Failed to retrieve suit data. Please try again.");
            ...
            Logger.getLogger(SuitsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
