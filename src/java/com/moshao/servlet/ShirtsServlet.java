...
public class ShirtsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            List<Product> shirts = productDao.getShirts();
            // Set shirts attribute in request for JSP rendering
            request.setAttribute("shirts", shirts);
            // Forward request to shirts.jsp for rendering
            request.getRequestDispatcher("shirts.jsp").forward(request, response);
        } catch (SQLException e) {
            // Log SQL exceptions
            logger.log(Level.SEVERE, "SQL Exception", e);
            // Handle database error
            response.getWriter().println("Failed to retrieve shirt data. Please try again.");
            ...
            Logger.getLogger(ShirtsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
